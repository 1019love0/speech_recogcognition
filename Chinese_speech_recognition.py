# -*- coding: utf-8 -*-
"""
Created on Tue Apr 16 19:01:26 2019
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
    ...
    saver.restore(sess,kpt)
    section='\n{0:=^40}\n'
    print(section.format('Testing epoch'))
    ...
    for batch in range(n_batches_per_epoch):
        print('开始获得测试数据：'+str(batch))
        next_idx,source,source_lengths,sparse_labels=next_batch(wav_files,labels,next_idx,batch_size)
        print("结束了")
        #print(source)
        #print(sparse_labels)
        feed={input_tensor:source,targets:sparse_labels,seq_length:source_lengths,keep_dropout:1.0}
